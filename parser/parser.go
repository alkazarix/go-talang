...
func (p *Parser) classDeclaration() ast.Stmt {
	...
	p.expect(token.RightBrace, fmt.Sprintf("expected '}' after class block. got '%s'", p.tokens[p.current].Literal))
...
func (p *Parser) returnStatement() ast.Stmt {
	...
	if !p.match(token.Semicolon) {
		...
		p.expect(token.Semicolon, fmt.Sprintf("expected ';' after return value: got %s", p.tokens[p.current].Literal))
	}
	return stmt
}
...
func (p *Parser) primary() (expr ast.Expr) {
	...
	if p.match(token.LeftBracket) {
		...
		p.expect(token.RightBracket, "expected  ']' after array.")
